...
function initializeAnimations() {
    ...
    setupImageColorToggle();
}

function setupImageColorToggle() {
    const images = document.querySelectorAll('.bw-image');

    images.forEach((img) => {
        img.addEventListener('click', () => {
            images.forEach((i) => i.classList.remove('color'));
            img.classList.add('color');
        });
    });
}

// Add click animation on images (also used before opening modal)
function addImageClickAnimation() {
    const imgs = document.querySelectorAll('.bw-image, .photo-container img');
    imgs.forEach(img => {
        img.addEventListener('click', () => {
            // trigger CSS animation by toggling class
            img.classList.remove('clicked-anim');
            // force reflow to restart animation
            void img.offsetWidth;
            img.classList.add('clicked-anim');
            // remove class after animation ends to keep DOM clean
            setTimeout(() => img.classList.remove('clicked-anim'), 500);
        });
    });
}

// initialize extra behaviors after DOM ready
document.addEventListener('DOMContentLoaded', () => {
    addImageClickAnimation();
});

// Image modal handling: open clicked image full-screen in color
document.addEventListener('click', (e) => {
    const target = e.target;
    if (target.matches('.bw-image, .photo-container img')) {
        const modal = document.getElementById('image-modal');
        const modalImg = document.getElementById('modal-img');
        const caption = document.getElementById('modal-caption');

        // Set full-size image and caption
